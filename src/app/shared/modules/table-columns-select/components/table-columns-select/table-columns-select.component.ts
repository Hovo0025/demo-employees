...
import { TableColumnI } from '@shared/interfaces';

@Component({
  selector: 'demo-table-columns-select',
...
export class TableColumnsSelectComponent implements OnInit {
  ...
  getColumns(): void {
    this.allColumnsKey = this.allDisplayedColumns.filter((col: TableColumnI) => !col.hide).map((col) => col.key);
